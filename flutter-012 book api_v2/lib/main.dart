...
// search 라는 매개변수에 전달된 값을 받아서
  void searchBox(String search) {
    if (search.isEmpty) return;
    // NaverOpenAPI().loadBooks()에게 전달하고, 검색어에 해당하는 결과를 return 받기
    Future<List<NaverBookDto>>? searchresultBooks =
        NaverOpenAPI().loadBooks(search);
    // return 받은 결과를 resultBooks state 변수에 반영하여 List 화면 다시 그리기
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: SearchBox(searchBooks: searchBox),
      ),
...
class ViewlListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: FutureBuilder(
        future: resultBooks,
        // FutureBuilder가 resultBooks 데이터를 사용하여 List 데이터로 변환을 하고,
        // 그 데이터를 다시 ListView.builder에게 전달하여
        // 실제 화면에 보여질 List를 생성한다.
        builder: (context, snapshot) {
          return ListView.builder(
            itemCount: snapshot.data!.length,
            // ListView.builder는 리스트의 각 항목 (한개 데이터)들을
            // ListTile에게 전달하여 한개 데이터를 표현할 View를 만들고
            // 그들을 모아서 리스트로 보여준다
